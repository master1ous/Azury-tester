...
if(!data) return;
if(!data.sticky_messages[0]) return;
if(message.author.bot) return;

    datas.forEach(async respond => {
        if(message.channel.id !== respond.sticky_messages[0].channel) return;

        const channelsa = client.channels.cache.get(respond.sticky_messages[0].channel)
        if(!channelsa) return;

        console.log("Got for each")
    
        if(respond.sticky_messages[0].message){
            console.log("Found an id")
            client.guilds.cache.get(message.guild.id).channels.cache.get(message.channel.id).messages.fetch(respond.sticky_messages[0].message).then(async (m: any) => {
                m.delete()
                console.log("Should be delete")
            }).catch(() => { })

if(!respond.sticky_messages[0].btnUrl && !respond.sticky_messages[0].btnLabel && !respond.sticky_messages[0].btnEmoji){
...
if(respond.sticky_messages[0].btnUrl && respond.sticky_messages[0].btnLabel && !respond.sticky_messages[0].btnEmoji){
    ...
    message.channel.send({content: `${respond.sticky_messages[0].content}`, components: [
        new ActionRowBuilder<ButtonBuilder>()
                    .addComponents([
                        new ButtonBuilder()
					.setLabel(`${respond.sticky_messages[0].btnLabel}`)
                    .setURL(`${respond.sticky_messages[0].btnUrl}`)
					.setStyle(ButtonStyle.Link),
                    ])
    ]}).then((msg) => {
        console.log("Should be connect")

        model.findOneAndUpdate({ guildID: message.guild.id, "sticky_messages.channel": message.channel.id }, { $set: { "sticky_messages.$.content": msg.content, "sticky_messages.$.message": msg.id, "sticky_messages.$.channel": message.channel.id, "sticky_messages.$.btnUrl": respond.sticky_messages[0].btnUrl, "sticky_messages.$.btnLabel": respond.sticky_messages[0].btnLabel} }, { new: true }).exec()
    })
}
